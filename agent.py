...
- Provide clear, concise responses"""
    ...
    def run_single_prompt(self, prompt: str):
        ...
        # Check cache (disabled for now as Gemini responses vary)
        # cache_key = f"prompt:{prompt}"
        # cached_response = self.cache.get(cache_key)
        # if cached_response:
        #     print(f"[CACHED] {cached_response}")
        #     print(f"\nExecution time: {time.perf_counter() - start_time:.3f}s (from cache)")
        #     return
        ...
        # Cache response (disabled as responses vary)
